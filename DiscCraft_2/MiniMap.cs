        ...
        public static void Draw(SpriteBatch spriteBatch)
        {
            ...
            for (int i = 0; i < Handler.chunks.Count; i++)
            {
                ...
                if (Handler.chunks[chunk.Key].isDrawed)
                    spriteBatch.Draw(Main.Bounds, new Rectangle((int)chunk.Key.X * 16 + 400, (int)chunk.Key.Y * 16 + 16, 16, 16), color);
                else
                    spriteBatch.Draw(Main.Bounds, new Rectangle((int)chunk.Key.X * 16 + 400, (int)chunk.Key.Y * 16 + 16, 16, 16), Color.Green);
