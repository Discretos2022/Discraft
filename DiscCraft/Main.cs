...
namespace DiscCraft
{
    public class Main : Game
    {
        ...
        public BlockV3[,,] block3List = new BlockV3[8, 5, 8];          ///    new BlockV3[8, 320, 8];
        ...
        protected override void Initialize()
        {
            ...
            //camera = new Camera(this, new Vector3(10, 8, 10), new Vector3(0, 0, 0), 0.5f);

            //cameraV2 = new CameraV2(GraphicsDevice, Window);
            cameraV2 = new Camera(this, new Vector3(0,0,0), new Vector3(0,0,0), 0.2f); // 0.5f

            vertexBuffer = new VertexBuffer(GraphicsDevice, typeof(VertexPositionTexture), 100000, BufferUsage.WriteOnly);
            ...
            //for (int x = 0; x < block3List.GetLength(0); x++)
            //{
            //    for (int y = 0; y < block3List.GetLength(1); y++)
            //    {
            //        for (int z = 0; z < block3List.GetLength(2); z++)
            //        {
            //            block3List[x, y, z] = new BlockV3(new Vector3(x, y, z), new Vector3(x, y, z), chunk);
            //        }
            //    }
            //}

            chunk = new Chunk(new Vector3(10, 10, 10));
            chunk.Generate();

            Handler.Init();

            for (int x = 0; x < Handler.chunks.GetLength(0); x++)
            {
                for (int z = 0; z < Handler.chunks.GetLength(1); z++)
                {
                    if (Handler.chunks[x, z] == null)
                        Handler.chunks[x, z] = new Chunk(new Vector3(x * 16, 0, z * 16));

                    Handler.chunks[x, z].Generate();

                }
            }


            blockV4 = new BlockV4(new Vector3(0,0,0), new Vector3(0,0,0), chunk); 
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            //worldMatrix = Matrix.CreateWorld(new Vector3(10,10,10), Vector3.Forward, Vector3.Up);
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            blockV4.SetVertexData(vertexBuffer);
            ...
            //if (xMax > block3List.GetLength(0))
            //    xMax = block3List.GetLength(0);

            //if (yMax > block3List.GetLength(1))
            //    yMax = block3List.GetLength(1);

            //if (zMax > block3List.GetLength(2))
            //    zMax = block3List.GetLength(2);
            ...
            //        {
            //            if (block3List[x, y, z] != null)
            //            {

            //                block3List[x, y, z].SetTexture(GrassSide, GrassSide, GrassSide, GrassSide, GrassUp, GrassDown);
            //                block3List[x, y, z].Draw(GraphicsDevice, basicEffect, block3List, cameraV2.Position);
            //                block3List[x, y, z].isSelected = false;
            //            }
            //        }
            //    }
            //}

            //chunk.Draw(cameraV2, GraphicsDevice, basicEffect);

            //blockV4.Draw(GraphicsDevice, basicEffect, block3List, cameraV2.Position);

            Handler.Draw(cameraV2, GraphicsDevice, basicEffect);


            VertexPositionColor[] vertax = new VertexPositionColor[2];


            vertax[0] = new VertexPositionColor(new Vector3(0,0,0), Color.Red);
            vertax[1] = new VertexPositionColor(new Vector3(10,0,0), Color.Green);

            basicEffect.TextureEnabled = false;
            basicEffect.VertexColorEnabled = true;


            //GraphicsDevice.DrawUserPrimitives(PrimitiveType.LineList, vertax, 0, 1);
            ...
            //cameraV2.CameraUi(0);
            //cameraV2.CameraType(CameraV2.CAM_TYPE_OPTION_FIXED);

            //Ray ray = new Ray(cameraV2.Position, cameraV2.LookAtDirection);
            ...
            Vector3 cursorEnd = new Vector3(Maxdist, 0, 0);

            /// Build a rotation matrix
            //Matrix rotationMatrix = cameraV2.World;    //Matrix.CreateRotationX(cameraV2.LookAtDirection.X) * Matrix.CreateRotationY(cameraV2.LookAtDirection.Y); // * Matrix.CreateRotationZ(cameraRotation.Z);

            Matrix rotationMatrix = Matrix.CreateRotationX(cameraV2.cameraRotation.X) * Matrix.CreateRotationY(cameraV2.cameraRotation.Y);

            //cameraV2.cameraRotation.Y += 0.001f;

            /// Build look at offset vector
            //cursorEnd = Vector3.Transform(new Vector3(0, 0, -Maxdist), rotationMatrix);
            cursorEnd = Vector3.Transform(new Vector3(0, 0, Maxdist), rotationMatrix);
            //cursorEnd += cursorOrigine;
            ...
            for (int p = 0; p < (100 * Maxdist); p++)
            {
                Vector3 point = cursorOrigine + ((cursorEnd / (100 * Maxdist)) * p);

                Vector3 pointInBlock = new Vector3(Util.UpperIntegerPlus(point.X), Util.UpperIntegerPlus(point.Y), Util.UpperIntegerPlus(point.Z)) - Handler.chunks[0, 0].Position;

                bool isOut = false;

                if (pointInBlock.X < 0) isOut = true;
                if (pointInBlock.Y < 0) isOut = true;
                if (pointInBlock.Z < 0) isOut = true;

                if (pointInBlock.X >= Handler.chunks[0, 0].GetBlockList().GetLength(0)) isOut = true;
                if (pointInBlock.Y >= Handler.chunks[0, 0].GetBlockList().GetLength(1)) isOut = true;
                if (pointInBlock.Z >= Handler.chunks[0, 0].GetBlockList().GetLength(2)) isOut = true;


                if (!isOut && Handler.chunks[0, 0].GetBlockList()[(int)pointInBlock.X, (int)pointInBlock.Y, (int)pointInBlock.Z] != null)
                {

                    Handler.chunks[0, 0].GetBlockList()[(int)pointInBlock.X, (int)pointInBlock.Y, (int)pointInBlock.Z].isSelected = true;

                    collisionText = "collision : true  block : " + (int)pointInBlock.X + " ; " + (int)pointInBlock.Y + " ; " + (int)pointInBlock.Z;

                    //Console.WriteLine(collisionText);
                    ...
                    if (MouseInput.getMouseState().LeftButton == ButtonState.Pressed && MouseInput.getOldMouseState().LeftButton != ButtonState.Pressed)
                    {
                        ...
                        Handler.chunks[0, 0].GetBlockList()[(int)pointInBlock.X, (int)pointInBlock.Y, (int)pointInBlock.Z] = null;
                    }
        ...
        L_1:
            ...
            spriteBatch.DrawString(UltimateFont, "view end : " + cursorEnd, new Vector2(10, 210), Color.White);
            ...
            Vector2 BasePos = new Vector2(650, 350);
            int Scale = 10;

            /// graph 1
            Render.DrawLineV1_1(Bounds, new Vector2(0, -10) * Scale + BasePos, new Vector2(0, 10) * Scale + BasePos, spriteBatch, Color.Black, 2, Render.LineType.Center);
            Render.DrawLineV1_1(Bounds, new Vector2(-10, 0) * Scale + BasePos, new Vector2(10, 0) * Scale + BasePos, spriteBatch, Color.Black, 2, Render.LineType.Center);
            /// camera
            Render.DrawLineV1_1(Bounds, new Vector2(cursorOrigine.X, cursorOrigine.Z) * Scale + BasePos, new Vector2(cursorEnd.X, cursorEnd.Z) * Scale + BasePos, spriteBatch, Color.Red, 2, Render.LineType.Center);
            /// Face 1 block 1
            Render.DrawLineV1_1(Bounds, new Vector2(0, 0) * Scale + BasePos, new Vector2(1, 0) * Scale + BasePos, spriteBatch, Color.Yellow, 2, Render.LineType.Center);
            Render.DrawLineV1_1(Bounds, new Vector2(1, 0) * Scale + BasePos, new Vector2(1, 1) * Scale + BasePos, spriteBatch, Color.Yellow, 2, Render.LineType.Center);



            BasePos = new Vector2(650, 120);
            Scale = 10;
            ...
            Render.DrawLineV1_1(Bounds, new Vector2(-10, 0) * Scale + BasePos, new Vector2(10, 0) * Scale + BasePos, spriteBatch, Color.Black, 2, Render.LineType.Center);
            /// camera
            Render.DrawLineV1_1(Bounds, new Vector2(cursorOrigine.X, -cursorOrigine.Y) * Scale + BasePos, new Vector2(cursorEnd.X, -cursorEnd.Y) * Scale + BasePos, spriteBatch, Color.Red, 2, Render.LineType.Center);
            /// Face 1 block 1
            Render.DrawLineV1_1(Bounds, new Vector2(0, 0) * Scale + BasePos, new Vector2(1, 0) * Scale + BasePos, spriteBatch, Color.Yellow, 2, Render.LineType.Center);
            Render.DrawLineV1_1(Bounds, new Vector2(0, 0) * Scale + BasePos, new Vector2(0, -1) * Scale + BasePos, spriteBatch, Color.Yellow, 2, Render.LineType.Center);
